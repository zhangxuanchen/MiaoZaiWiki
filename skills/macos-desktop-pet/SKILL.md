...
`DISCLAIMER.md` 是这份 skill 的**合规材料**，对应最高法
《关于依法审理涉人工智能纠纷案件的意见》（法发〔2026〕10号）**第十三条** ——
「免费开源 + 公开说明其功能和安全风险」是获得该条免责可能性的必要条件之一。
**分发时不要删。**

一句话版本：**这份 skill 只给方法，不给素材。** 它不含任何美术素材 / 字体 / 模型，
产出的形象、名称、图标、文案是否合规，**由使用者自行核实**（文档里有查法：
依赖许可、商标检索、素材来源、以图搜图）。
