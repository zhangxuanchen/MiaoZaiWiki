...
到 [**Releases 页面**](https://github.com/zhangxuanchen/MiaoZaiWiki/releases) 下载 **`喵藏-1.1.0.dmg`**，双击打开，把 **喵藏.app** 拖进「应用程序」：
...
**分层加权关键词匹配**（零 AI，确定可复现）——把文章切成四层信号，各按可信度加权：

| 信号层 | 权重 | 说明 |
|---|---|---|
| 标题 | **×5** | 作者亲手写的主题概括，最可信 |
| 摘要（og:description） | ×3 | 人写的一句话主旨 |
| 大纲（h1-h3） | ×2 | 作者的分节主题（手动笔记没有这层） |
| 正文（前 6000 字） | ×1 | 只作佐证，大文档也不空转 |

- 英文词按**单词边界**匹配（`go` 不误伤 `google`、`sql` 不匹配 `mysql`），中文按子串
- 每个词最多计 5 次（防单词刷爆），每命中一个**不同的**词额外 +2（命中面广更"像"）
- **全 0 分 → 落到「收件箱」**（兜底，不是失败）
- 分数相同时按**类名的字符编码**从大到小决胜（`sort` 比的是 `(分数, 类名)` 元组）
  —— 这是个隐藏耦合：**改分类名字会改变平票结果**
- 默认 **7 类**：AI与Agent / 编程开发 / 产品与商业 / 效率与工具 / 设计与审美 / 科学与科普 / 生活与随笔
- 已抓过的 URL 自动跳过，提示「已经吃过了」
- 改词表**只对之后新喂的生效**；`--rescan` 不重新分类（它只认 front-matter 和文件夹名）
- 词表调整细节见 `dist/miao-zang/references/classify.md`
