...
// MARK: - 配置
struct PetConfig {
    static var configPath: String { NSString("~/.catpet/config.json").expandingTildeInPath }
...
final class CatView: NSView {
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        if event.clickCount == 2 {
            // 双击 = 打开**应用内**的总目录（以前是打开书库文件夹）。
            // 要看原始文件就走总目录窗口右上角那个文件夹图标按钮
            // （右键菜单里原来也有一条，跟它是重复入口，已经撤掉）。
            state = .wave
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    private var panels: [String: PetPanel] = [:]     // petID → 悬浮窗
    private var views: [String: CatView] = [:]       // petID → 猫
    private var timer: Timer?

    // MARK: 两只猫挨得够近就一起冒心
    static let buddyGapMax: CGFloat = 10        // 水平间隙阈值（pt）
    static let buddyMinVOverlap: CGFloat = 28   // 竖直方向至少要叠这么多，才算"并排"
    private var buddyHeartAt: TimeInterval = 0
    private var nextBuddyHeartAt: TimeInterval = 0
    private var buddyWasNear = false

    /// 两个窗口之间的水平间隙（重叠算 0）和竖直重叠量。抽成 static 便于单测。
    static func buddyMetrics(_ a: NSRect, _ b: NSRect) -> (gap: CGFloat, vOverlap: CGFloat) {
        let gap = max(max(a.minX - b.maxX, b.minX - a.maxX), 0)
        let vOverlap = min(a.maxY, b.maxY) - max(a.minY, b.minY)
        return (gap, vOverlap)
    }
    static func buddyIsNear(_ a: NSRect, _ b: NSRect) -> Bool {
        let m = buddyMetrics(a, b)
        return m.gap <= buddyGapMax && m.vOverlap >= buddyMinVOverlap
    }

    func applicationDidFinishLaunching(_ notification: Notification) {
        // 命令行自检入口：不用点菜单也能读写登录项状态（方便脚本验证）
        if CommandLine.arguments.contains("--loginitem") {
            runLoginItemCLI()
            return
        }
        // --selftest：打印"我能不能写配置 / 各只猫解析到哪个书库"
        if CommandLine.arguments.contains("--selftest") {
            runSelfTestCLI()
            exit(0)     // 双保险：这两个分支都在"单实例保护"**之前**，
                        // 漏了退出就会原地变成第二只猫（真的踩过一次）
        }
        // --check-lib：立刻按**自动巡检的同一套判据**看一眼每只猫的书库（纯读，不落盘、不同步）。
        // 巡检是 50~70 分钟一次，等它太慢 —— 想确认"我改了文件它认不认"，跑这个。
        if CommandLine.arguments.contains("--check-lib") {
            runLibraryCheckCLI()
            exit(0)
        }
        // 单实例保护：跑起两个进程会出现「桌面上两只猫抢同一个书库」，
        // 那种重复不是这个功能想要的「两只猫」，所以后起的直接退出。
        ...
        // 加猫 / 送猫之后，由 CatView 发通知回来重建窗口
        NotificationCenter.default.addObserver(
            forName: .catpetPetsChanged, object: nil, queue: .main
        ) { [weak self] _ in self?.syncWindows() }

        syncWindows()

        // 配置被人手改出问题时说明情况（都只从内存里搁置，文件原样保留，不静默丢）
        let store = PetStore.shared
        // 顺手修掉一个漏翻：这几条以前是**裸中文**，而它们真的会显示在气泡里（不是只打日志）
        var notes: [(key: String, args: [CVarArg])] = []
        if store.droppedOnLoad > 0 {
            notes.append(("多了 %d 只喵（最多养 %d 只）", [store.droppedOnLoad, PetStore.maxPets]))
        }
        if store.duplicateOnLoad > 0 {
            notes.append(("有 %d 只的书库目录和别的一样", [store.duplicateOnLoad]))
        }
        if !notes.isEmpty, let first = store.pets.first, let v = views[first.id] {
            // 这里在 AppDelegate 层，没有"当前猫"的上下文 → 借承载这只气泡的猫的语言来拼
            let lang = v.petLang
            func trLang(_ zh: String) -> String { lang == .en ? (STRINGS_EN[zh] ?? zh) : zh }
            let lines = notes.map { String(format: trLang($0.key), arguments: $0.args) }
            let text = trLang("配置有点问题，先没摆出来：\n") + lines.joined(separator: "\n")
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                v.showBubbleLiteral(text, duration: 9)
            }
        }
        ...
        let t = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { [weak self] _ in
            guard let self = self else { return }
            let now = CACurrentMediaTime()
            self.updateBuddy(now: now)
            for v in self.views.values { v.needsDisplay = true }
            PetStore.shared.flushIfNeeded(now: now)
        }
        RunLoop.main.add(t, forMode: .common)
        timer = t
    }

    /// --loginitem / --loginitem on / --loginitem off：打印或切换开机自启后退出
    private func runLoginItemCLI() {
        guard #available(macOS 13.0, *) else { print("status=unsupported"); exit(0) }
        let svc = SMAppService.mainApp
        let args = CommandLine.arguments
        if let i = args.firstIndex(of: "--loginitem"), i + 1 < args.count {
            let act = args[i + 1]
            do {
                if act == "on" {
                    try svc.register()
                    print("register ok -> status=\(svc.status.rawValue)")
                } else if act == "off" {
                    try svc.unregister()
                    print("unregister ok -> status=\(svc.status.rawValue)")
                } else {
                    print("usage: --loginitem [on|off]")
                }
            } catch {
                print("error=\(error.localizedDescription) code=\((error as NSError).code) domain=\((error as NSError).domain)")
            }
        } else {
            print("status=\(svc.status.rawValue) (0=notRegistered 1=enabled 2=requiresApproval 3=notFound)")
        }
        exit(0)
    }

    /// --check-lib：按**自动巡检的同一套判据**看一眼每只猫的书库。
    /// 纯读 —— 不写配置、不跑同步，随便跑。想看"下次巡检会不会同步"，跑这个就知道。
    private func runLibraryCheckCLI() {
        print("书库巡检判据（纯读：不落盘、不同步）")
        print(String(format: "  巡检间隔 = %.0f~%.0f 分钟随机（平均一小时一次）",
                     CatView.libCheckRange.lowerBound / 60, CatView.libCheckRange.upperBound / 60))
        for p in PetStore.shared.pets {
            guard let now = CatView.libraryHash(ofRoot: p.rootPath) else {
                print("  \(p.name)：读不到目录 \(p.rootPath)")
                continue
            }
            let kept = p.libHash.isEmpty ? "(空 · 还没记过)" : String(p.libHash.prefix(12)) + "…"
            let verdict: String
            switch CatView.libraryCheck(stored: p.libHash, current: now) {
            case .recordOnly: verdict = "第一次见 → 只记账，不同步"
            case .unchanged:  verdict = "没变化 → 什么都不做"
            case .resync:     verdict = "有变化 → 下次巡检会同步"
            }
            print("  \(p.name)：存=\(kept)  算=\(String(now.prefix(12)))…")
            print("        → \(verdict)")
        }
        exit(0)
    }

    /// --selftest：把「能不能写配置」和「每只猫解析到哪个书库」打出来。
    /// 写保护那层加错的话会非常隐蔽（app 自己都存不了设置了），所以留个一眼可验的入口。
    private func runSelfTestCLI() {
        print("bundleID      = \(Bundle.main.bundleIdentifier ?? "(nil)")")
        print("configPath    = \(PetConfig.configPath)")
        print("mayWriteConfig= \(PetStore.mayWriteConfig)")
        print("python        = \(PetConfig.pythonPath)  存在=\(FileManager.default.isExecutableFile(atPath: PetConfig.pythonPath))")
        for p in PetStore.shared.pets {
            let ok = FileManager.default.fileExists(atPath: p.rootPath)
            let h = p.libHash.isEmpty ? "(还没记)" : String(p.libHash.prefix(8)) + "…"
            print("  pet \(p.name)  look=\(p.look) eye=\(p.eye) bib=\(p.bib) ear=\(p.ear)"
                  + " tail=\(p.tail)(\(TailStyle.clamp(p.tail).name))"
                  + " mouth=\(p.mouth)(\(MouthStyle.clamp(p.mouth).name))"
                  + " eyeColor=\(p.eyeColor)(\(EyeColor.clamp(p.eyeColor).name)) lang=\(p.lang)"
                  + "  书库指纹=\(h)  root=\(p.rootPath)  存在=\(ok)")
        }
        let problems = PetStore.shared.healthReport()
        if problems.isEmpty {
            print("  自检：一切正常 ✓")
        } else {
            print("  自检发现问题：")
    ...
    func applicationWillTerminate(_ notification: Notification) {
        PetStore.shared.save()          // 把最后的位置／改名落盘
    }

    /// 同一 bundle id 是否已经有别的实例在跑。
    /// 用 `swift main.swift` 跑预览脚本时 bundleIdentifier 是 nil，直接跳过检查。
    ...
    /// 两只猫挨得够近 → 不定时一起冒心。
    /// 心是由**各自的窗口**画的（两颗同步出现，合起来才是「❤️❤️」），
    /// 所以这里只负责判断距离 + 统一发令，不负责绘制。
    private func updateBuddy(now: TimeInterval) {
        let frames: [(String, NSRect)] = PetStore.shared.pets.compactMap {
            guard let w = panels[$0.id] else { return nil }
            return ($0.id, w.frame)
        }
        guard frames.count == 2 else { resetBuddy(); return }
        let (aID, a) = frames[0], (bID, b) = frames[1]

        guard AppDelegate.buddyIsNear(a, b) else { resetBuddy(); return }

        // 谁在左谁在右 —— 心往内侧飘；再把"伙伴高我多少"发下去，
        // 斜着摆的时候心才能朝对方靠拢，不会一高一低。
        let leftIsA = a.minX <= b.minX
        let dy = b.midY - a.midY
        views[leftIsA ? aID : bID]?.buddySide = 1      // 伙伴在右边 → 自己画在右侧
        views[leftIsA ? bID : aID]?.buddySide = -1
        views[aID]?.buddyDy = dy
        views[bID]?.buddyDy = -dy

        if !buddyWasNear {
            // 刚凑近：很快先冒一次，像"发现对方了"
            buddyWasNear = true
            nextBuddyHeartAt = now + Double.random(in: 0.4...1.2)
            return
        }
        guard now >= nextBuddyHeartAt else { return }
        buddyHeartAt = now
        nextBuddyHeartAt = now + Double.random(in: 2.5...7)   // 不定时
        for v in views.values { v.buddyHeartAt = now }        // 两边同步
    }

    private func resetBuddy() {
        buddyWasNear = false
        nextBuddyHeartAt = 0
        for v in views.values where v.buddySide != 0 || v.buddyHeartAt != 0 || v.buddyDy != 0 {
            v.buddySide = 0
            v.buddyDy = 0
            v.buddyHeartAt = 0
        }
    ...
    /// 让窗口数量和 PetStore 里的猫保持一致（新增的建、没了的拆）
    private func syncWindows() {
        let store = PetStore.shared
        for id in Array(panels.keys) where store.pet(id: id) == nil {
            panels[id]?.orderOut(nil)
            panels[id] = nil
            views[id] = nil
        }
        // prefix() 是第三层保险：即使 store 里混进了多余项，也不会摆出第三只
        for pet in store.pets.prefix(PetStore.maxPets) where panels[pet.id] == nil {
            makeWindow(for: pet)
        }
    }

    private func makeWindow(for pet: PetProfile) {
        let W: CGFloat = 186, H: CGFloat = 205
        let panel = PetPanel(
        ...
        panel.hasShadow = false          // 背景纯透明：不要系统窗口阴影
        ...
        let view = CatView(frame: NSRect(x: 0, y: 0, width: W, height: H))
        view.petID = pet.id
        view.scheduleLibraryCheck()   // 每小时左右随机看一眼书库，变了就自动同步
        view.captureLibraryBaselineIfNeeded()   // 先立个基线（没记过才写）
        view.autoresizingMask = []
        panel.contentView = view
        // 背景纯透明：不加任何 CALayer 阴影 / 光晕，只留猫本身
        view.wantsLayer = true
        view.layer?.backgroundColor = NSColor.clear.cgColor
        view.layer?.shadowOpacity = 0

        let origin = initialOrigin(for: pet)
        panel.setFrameOrigin(origin)
        PetStore.shared.update(id: pet.id) {
            $0.x = Double(origin.x); $0.y = Double(origin.y)
        }

        // 拖动结束后把位置记到这只猫名下（限频落盘在 update/flushIfNeeded 里做）
        NotificationCenter.default.addObserver(
            forName: NSWindow.didMoveNotification, object: panel, queue: .main
        ) { [weak self] _ in
            guard let p = self?.panels[pet.id] else { return }
            let o = p.frame.origin
            PetStore.shared.update(id: pet.id) { $0.x = Double(o.x); $0.y = Double(o.y) }
        }
    ...
    /// 还没摆过的猫的初始位置。
    /// 第一只沿用老版本存在 UserDefaults 里的坐标（升级后猫不会突然跳走）；
    /// 后面的按序号往左错开，免得两只猫叠在一起，看起来还是"只有一只"。
    private func initialOrigin(for pet: PetProfile) -> NSPoint {
        if pet.x != 0 || pet.y != 0 { return NSPoint(x: pet.x, y: pet.y) }
        let idx = PetStore.shared.index(id: pet.id) ?? 0
        if idx == 0 {
            let d = UserDefaults.standard
            let lx = d.double(forKey: "catpet.x"), ly = d.double(forKey: "catpet.y")
            if lx != 0 || ly != 0 { return NSPoint(x: lx, y: ly) }
        }
        guard let screen = NSScreen.main else { return NSPoint(x: 220, y: 220) }
        let f = screen.visibleFrame
        return NSPoint(x: f.maxX - 190 - CGFloat(idx) * 200, y: f.minY + 30)
    }

    /// 装一个最小主菜单。
    /// 关键：**必须有「编辑」菜单**，否则 ⌘C/⌘V/⌘X/⌘A 在 NSTextView 里按不出来——
    /// macOS 的复制粘贴是靠主菜单项的 keyEquivalent 走 responder chain 派发的，
    /// 没有菜单项就没有这个快捷键，accessory app 尤其容易踩。
    /// 菜单项 target 一律留 nil，这样动作才能沿着响应链找到当前第一响应者。
    /// 抽出成 internal 便于单测脚本直接调用
    func installMainMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: String(format: tr("关于%@"), appName()),
                        action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)),
                        keyEquivalent: "")
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: String(format: tr("隐藏%@"), appName()),
                        action: #selector(NSApplication.hide(_:)),
                        keyEquivalent: "h")
        appMenu.addItem(withTitle: String(format: tr("退出%@"), appName()),
                        action: #selector(NSApplication.terminate(_:)),
                        keyEquivalent: "q")
        appItem.submenu = appMenu

        // 编辑菜单（复制/粘贴能用的关键）
        // 注意：⌘C / ⌘V / ⌘X 是靠**菜单项的 keyEquivalent** 沿 responder chain 派发的，
        // 所以剪切/拷贝/粘贴/删除/全选一个都不能少 —— 之前漏了前四项，
        // 结果输入框里怎么按 ⌘V 都没反应（menu 里根本没有 paste: 这个快捷键）。
        ...
        editMenu.addItem(withTitle: tr("删除"), action: #selector(NSText.delete(_:)), keyEquivalent: "")
        editMenu.addItem(.separator())
...
applyLook(0)          // 启动前先套上默认形象；绘制里还会按每只猫重设
